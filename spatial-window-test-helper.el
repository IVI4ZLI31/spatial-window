...
(defun spatial-window--bts-row-heights (grid x-coords y-coords col-widths window-bounds)
  "Compute row heights for the grid.
GRID, X-COORDS, Y-COORDS, COL-WIDTHS, WINDOW-BOUNDS as in other helpers."
  (let* ((ncols (1- (length x-coords)))
         (nrows (1- (length y-coords)))
         (row-heights (make-vector nrows 0)))
    ;; Pass 1: single-row windows set minimum row height
    (dolist (entry window-bounds)
      (let* ((label (symbol-name (car entry)))
             (left (nth 1 entry)) (right (nth 2 entry))
             (top (nth 3 entry)) (bottom (nth 4 entry))
        ...
        (let* ((span-w (cl-loop for c in cols sum (aref col-widths c)))
               ;; Add internal separator widths for multi-col spans
               (total-w (+ span-w (1- (length cols))))
               (avail (- total-w 1))  ; 1-space left padding
               (content-lines (cons label (spatial-window--bts-wrap-dims dims avail)))
               (nlines (length content-lines)))
          (if (= (length rows) 1)
              ;; Single row: must fit all content
              (let ((r (car rows)))
                (when (> nlines (aref row-heights r))
                  (aset row-heights r nlines)))
            ;; Multi-row: handled in pass 2
            nil))))
    ;; Ensure minimum row height of 2
    (dotimes (r nrows)
      (when (< (aref row-heights r) 2)
        (aset row-heights r 2)))
      ...
      (let* ((label (symbol-name (car entry)))
        ...
        (when (> (length rows) 1)
          (let* ((span-w (cl-loop for c in cols sum (aref col-widths c)))
                 ...
                 (content-lines (cons label (spatial-window--bts-wrap-dims dims avail)))
                 (nlines (length content-lines))
                 ;; Current total height: sum of row heights + separator lines
                 (total-h (+ (cl-loop for r in rows sum (aref row-heights r))
                             (1- (length rows)))))
            (when (> nlines total-h)
              ;; Distribute extra height across rows
